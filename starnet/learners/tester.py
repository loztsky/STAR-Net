...
from starnet.utils.metrics import Evaluator
...
class Tester:
    ...
    def __init__(self, cfg, visualizer : TensorboardMultiLossVisualizer = None):
        self.cfg = cfg
    ...
    def predict(self, net, seq_loader, iteration=None, get_quali=False, add_temp=False, mode="val"):
        """
        ...
        with torch.no_grad():
            for i, sequence_data in enumerate(seq_loader):
                ...
                if self.cfg["dataset"] == "carrada":
                    ...
                    path_to_frames = os.path.join(self.paths['othr'], mode, seq_name[0]) # 数据集序列的目录路径，例如 D:\dataset\Radar\Carrada\2019-09-16-12-52-12
                ...
                for j, frame in enumerate(frame_dataloader):
                    ...
                    if self.cfg["n_Input"] == 1 and self.cfg["n_Output"] == 1:
                        rd_outputs = net(rd_data) # to do
                        rd_outputs = rd_outputs.to(self.device)
                        if self.sqflag: # rodnet  N x D x C x H x W
                    ...
                    if self.cfg["n_Output"] == 1:
                        ...
                        loss = torch.mean(torch.stack(rd_losses))
                    elif self.cfg["n_Output"] == 2:
                        if nb_losses < 3:
                    ...
                    if iteration and i == rand_seq:
                        if j == rand_frame:
                            ...
                            if self.cfg["n_Output"] == 2:
                                ...
                                self.visualizer.update_single_img_masks(rd_pred_grid, rd_gt_grid,
                                                                   iteration)
            self.test_results = dict()
            ...
            if self.cfg["n_Output"] == 1:
                ...
                # both range doppler and range angle
