...
class CarradaDataset(Dataset):
    ...
    def __init__(self, dataset, annotation_type, path_to_frames, process_signal,
        ...
        self.dataset = self.dataset[self.n_frames-1:]  # remove n first frames，以 n_frame - 1 为开始, 0 ,1 ,2, ..., n_frames - 1 为输入
        self.path_to_annots = self.path_to_frames / 'annotations' / self.annotation_type # 标签目录，例如 D:\dataset\Radar\Carrada\2019-09-16-12-52-12\annotations\${annotation_type}
    ...
    def __getitem__(self, idx):
        ...
        frame_names = [str(f_id).zfill(6) for f_id in range(frame_id - self.n_frames + 1, frame_id + 1)] # 记录第一帧后面所需要的帧数据的名称
        rd_matrices = list()
        ra_matrices = list()
        ad_matrices = list()
        # 下载多帧数据的第一张数据
        rd_mask = np.load(os.path.join(self.path_to_annots, init_frame_name,
                                       'range_doppler.npy')) # rd标签, D:\dataset\Radar\Carrada\2019-09-16-12-52-12\annotations\${annotation_type}\${init_frame_name}\range_doppler.npy
        ra_mask = np.load(os.path.join(self.path_to_annots, init_frame_name, 
                                       'range_angle.npy')) # ra标签, D:\dataset\Radar\Carrada\2019-09-16-12-52-12\annotations\${annotation_type}\${init_frame_name}\range_angle.npy
        for frame_name in frame_names:
            if self.process_signal:
                rd_matrix = np.load(os.path.join(self.path_to_frames,
                                                 'range_doppler_processed',
                                                 frame_name + '.npy')) # rd数据, D:\dataset\Radar\Carrada\2019-09-16-12-52-12\range_doppler_processed\${frame_name}.npy
                ra_matrix = np.load(os.path.join(self.path_to_frames,
                                                 'range_angle_processed',
                                                 frame_name + '.npy')) # ra数据, D:\dataset\Radar\Carrada\2019-09-16-12-52-12\range_angle_processed\${frame_name}.npy
                ad_matrix = np.load(os.path.join(self.path_to_frames,
                                                 'angle_doppler_processed',
                                                 frame_name + '.npy')) # ad数据, D:\dataset\Radar\Carrada\2019-09-16-12-52-12\angle_doppler_processed\${frame_name}.npy
            else:
                rd_matrix = np.load(os.path.join(self.path_to_frames,
                                                 'range_doppler_raw',
                                                 frame_name + '.npy')) # rd数据, D:\dataset\Radar\Carrada\2019-09-16-12-52-12\range_doppler_raw\${frame_name}.npy
                ra_matrix = np.load(os.path.join(self.path_to_frames,
                                                 'range_angle_raw',
                                                 frame_name + '.npy')) # ra数据, D:\dataset\Radar\Carrada\2019-09-16-12-52-12\range_angle_raw\${frame_name}.npy
                ad_matrix = np.load(os.path.join(self.path_to_frames,
                                                 'angle_doppler_raw',
                                                 frame_name + '.npy')) # ad数据, D:\dataset\Radar\Carrada\2019-09-16-12-52-12\angle_doppler_raw\${frame_name}.npy
            # 添加 idx - n_frames + 1, idx - n_frames, ..., idx ==> N x n_frames x 1 x H x W 
            rd_matrices.append(rd_matrix) 
            ra_matrices.append(ra_matrix)
            ad_matrices.append(ad_matrix)

        # Apply the same transfo to all representations, 随机数据增强
        ...
        rd_frame = {'matrix': rd_matrix, 'mask': rd_mask} # 输出的数据
        rd_frame = self.transform(rd_frame, is_vflip=is_vflip, is_hflip=is_hflip) # 转换
        if self.add_temp: # 如果为不为None或者False则要拓展
            if isinstance(self.add_temp, bool): # True 拓展第一维
                rd_frame['matrix'] = np.expand_dims(rd_frame['matrix'], axis=0)
            else:
                assert isinstance(self.add_temp, int) # 拓展第 add_temp 维度
...
class OTHRDataset(Dataset):
    ...
    def __init__(self, dataset, path_to_frames, process_signal,
                 n_frames, transformations=None, add_temp=False):
        self.dataset = dataset
        self.path_to_frames = Path(path_to_frames)
        self.process_signal = process_signal
        ...
        self.add_temp = add_temp
        self.dataset = self.dataset[self.n_frames - 1:]  # remove n first frames，以 n_frame - 1 为开始, 0 ,1 ,2, ..., n_frames - 1 为输入
        self.path_to_annots = self.path_to_frames / 'annotations' # 标签目录，例如 D:\dataset\Radar\Carrada\2019-09-16-12-52-12\annotations\${annotation_type}
    ...
    def __getitem__(self, idx):
        init_frame_name = self.dataset[idx][0].split(".")[0]
        frame_id = int(init_frame_name) # 使用数字进行填充
        frame_names = [str(f_id).zfill(6) for f_id in range(frame_id - self.n_frames + 1, frame_id + 1)] # 记录第一帧后面所需要的帧数据的名称
        rd_matrices = list()
        # ra_matrices = list()
        # ad_matrices = list()
        ...
        # 返回标签
        frame = {
                    'rd_matrix': rd_frame['matrix'], 
                    'rd_mask': rd_frame['mask']
                 }
