...
class TMVANet(nn.Module):
    ...
    def forward(self, x_rd, x_ra, x_ad):
        # Backbone
        ra_features, ra_latent = self.ra_encoding_branch(x_ra) # (N, 1, 5, 256, 256) ->  (1, 128, 64, 64)*2
        rd_features, rd_latent = self.rd_encoding_branch(x_rd) # (N, 1, 5, 256, 64)  ->  (1, 128, 64, 64)*2
        ad_features, ad_latent = self.ad_encoding_branch(x_ad) # (N, 1, 5, 256, 64)  ->  (1, 128, 64, 64)*2

        # ASPP blocks
        x1_rd = self.rd_aspp_block(rd_features) # (1, 128, 64, 64) ->
        x1_ra = self.ra_aspp_block(ra_features) # (1, 128, 64, 64) ->
        x1_ad = self.ad_aspp_block(ad_features) # (1, 128, 64, 64) ->
...
if __name__=="__main__":
    model = TMVANet(n_classes=4, n_frames=5).cuda()
    rd = torch.randn(1, 1, 5, 256, 64).cuda()
    ra = torch.randn(1, 1, 5, 256, 256).cuda()
    ad = torch.randn(1, 1, 5, 256, 64).cuda()
    output = model(rd, ra, ad)
