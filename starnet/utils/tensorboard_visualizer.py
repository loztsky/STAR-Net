...
class TensorboardMultiLossVisualizer:
    ...
    def update_train_loss(self, loss, losses, iteration):
        ...
        self.writer.add_scalar('train_losses/Dice', losses[1],
                               iteration)
    def update_single_train_loss(self, global_loss, rd_loss, rd_losses, iteration):
        """
        global loss : 0.5  * wCE + 0.5 * wSDice
        rd loss = global loss
        rd_losses:
            [0] : wCE
            [1] : wSDice
        iteration : the iter of this loss
        """
    ...
    def update_multi_train_loss(self, global_loss, rd_loss, rd_losses, ra_loss, ra_losses,
        ...
        """
        global loss : 0.5 * (0.5  * wCE + 0.5 * wSDice)_{rd} + 0.5 * (0.5  * wCE + 0.5 * wSDice)_{ra}
        rd loss = (0.5  * wCE + 0.5 * wSDice)_{rd}
        rd_losses:
            [0] : wCE of rd
            [1] : wSDice of rd
        ra loss = (0.5  * wCE + 0.5 * wSDice)_{ra}
        ra_losses:
            [0] : wCE of ra
            [1] : wSDice of ra
        iteration : the iter of this loss
    ...
    def update_val_metrics(self, metrics, iteration):
        ...
        self.writer.add_scalar('MIoU/Car',
                               ...
                               iteration)
    def update_single_val_metrics(self, metrics, iteration):
    ...
    def update_single_img_masks(self, rd_pred_grid, rd_gt_grid, iteration):
        self.writer.add_image('Range_Doppler/Predicted_masks', rd_pred_grid, iteration)
        self.writer.add_image('Range_Doppler/Ground_truth_masks', rd_gt_grid, iteration)